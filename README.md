...
> **强化等级与附魔等级的关系**：插件在装备**首次锻造**时记录它自带的原版附魔等级作为"底子"，
> 之后实际附魔等级 = 底子 + 强化等级。一把原版锋利 V 的剑内部强化等级达到 +3 后是
> **锋利 VIII**，Lore 显示 `[+8]`；一把无附魔的剑强化到 +3 则是 **锋利 III**，显示 `[+3]`。
> `[+N]` 始终显示当前主附魔等级；内部强化等级的计算语义不变，原版附魔也不会被强化覆盖掉。
> 强化等级归零（掉级到 0 或 `/hammr remove`）时装备会还原成锻造前的原版状态。
...
| 权限节点 | 说明 | 默认 |
| --- | --- | --- |
| `hammr.use` | 允许在铁砧上使用强化系统（玩家权限） | 所有人 |
| `hammr.command.set` | 允许使用 `/hammr set` | OP |
| `hammr.command.remove` | 允许使用 `/hammr remove` | OP |
| `hammr.command.give` | 允许使用 `/hammr give` | OP |
| `hammr.command.reload` | 允许使用 `/hammr reload` | OP |
| `hammr.admin` | 允许使用所有管理命令（包含以上全部节点） | OP |

> 取消某个权限组的 `hammr.use` 后，该组玩家在铁砧中不会看到强化预览，也无法执行强化。
...
- 修复光标上已有物品时成品覆盖导致的吞物品问题，背包操作改为下一 tick 执行并同步客户端
- 强化数据改为以 PDC 为唯一数据源，附魔等级 = 首次锻造时记录的原版底子 + 强化等级
- 锻造不再覆盖装备原有的原版附魔（锋利 V 强化 +1 得到锋利 VI，而不是被降成锋利 I）
- 强化归零 / `/hammr remove` 会把装备还原成锻造前的原版附魔状态
- 修复未锻造装备（原版附魔台/村民/指令获得）被误标记强化等级的问题
- 修复 +0 强化失败会清空装备原有原版附魔的问题
- 新增玩家权限节点 `hammr.use` 与 `hammr.command.*` 细分权限
- 铁砧爆炸半径改为读取 `explosion-radius` 配置
- 修复越级附魔豁免残留导致的原版铁砧突破附魔等级上限漏洞
- 修复校验未通过时金币已被扣除、以及扣款失败仍然执行强化的问题
- 修复缺少 Vault 时插件启用失败、经济插件晚注册导致永久免费强化的问题
- 收入账户改为缓存 + 异步预热，避免每次强化在主线程发起 UUID 查询
- 经验监听改为先做材质过滤，减少每颗经验球 / 每次挖掘的 ItemMeta 克隆
- 铁砧预览金币数与实际按等级扣费保持一致
